...
//   • Blind rotates — cross-map rotations begun after the enemy team committed
//     elsewhere but BEFORE any information existed (lib/demo/rotates.ts). The
//     information-anomaly counterpart to snap kills: it reads radar/wallhack
//     game-sense rather than aim. Blind-but-wrong rotates subtract.
...
import type { PlayerRotates } from "./rotates";
...
// Score one player from their in-match aim-quality stats plus (optionally)
// their rotation analysis (lib/demo/rotates.ts).
export function demoCheat(p: PlayerInsight, rot?: PlayerRotates | null): DemoCheat {
  ...
  const rotDisplay = rot
    ? `${rot.blindCorrect}✓${rot.blindWrong ? ` ${rot.blindWrong}✗` : ""}`
    : "—";
  const parts: [string, string, string, number | null, number][] = [
    ...
    ["rot", "Blind rotates", rotDisplay, sRot, 0.24],
  ...
  // Blind rotates ARE in the peak set: a radar user with clean aim should not
  // have their information tell averaged away by normal aim numbers.
  const core = present
    .filter((x) => ["snap", "hsacc", "rot"].includes(x[0]))
    .map((x) => x[3])
    .sort((a, b) => b - a);
  ...
  // Confidence scales with how many DIRECT tells we actually had (aim tells +
  // the rotation read). With none (old demo: HS%/reaction only) the score is
  // heavily discounted so a missing re-parse can't read as "clean" OR over-accuse.
  const direct = [sSnap, sAcc, sHsAcc, sRot].filter((x) => x != null).length;
