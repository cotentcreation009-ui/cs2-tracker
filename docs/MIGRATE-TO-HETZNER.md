# Migrating csrun.win from GCE to Hetzner Cloud

*Written 2026-09-18 from a live inventory of the `cs2` GCE VM. Supersedes §10 of
`DEPLOY-GCE-SINGLE-BOX.md`.*

## Why

The GCE box (e2-standard-2, 80 GB pd-balanced, static IP) costs ~$60/month and runs
at 2–4 % CPU / 2.1 GB of 8 GB RAM. A Hetzner CPX31 (4 vCPU, 8 GB, 160 GB NVMe, 3 TB
traffic, Ashburn) is ~$17/month for a bigger machine. Everything is docker-compose
behind Cloudflare, so the move is a copy plus a DNS edit.
...
| Poster art | `~/csrun/assets-cs2` **17 GB**, `backdrops`, `emblems`, `map-icons`, `gallery`, `masters`, `data/csrun.db` | 17 GB | rsync GCE → Hetzner directly (~$2 GCE egress). Bucket copy would be the same bytes, slower |
| Backups | `~/backups/*.sql.gz` (14 days) + GCS bucket | 261 MB | last 3 copied; new cron targets R2 or Hetzner Backups |
| Firewall | GCE rule `allow-cf-web` (80/443 from Cloudflare only) | — | reproduced by `deploy/hetzner/bootstrap.sh` with ufw |

## Order of operations

1. **Owner**: Hetzner account → project → add the operator's SSH public key → create
   server (Ashburn, Ubuntu 24.04, CPX31, IPv4+IPv6, Backups on). Send the IPv4.
2. **Operator**: `scp deploy/hetzner/bootstrap.sh root@NEW:` and run it. Docker, `cs2`
   user, ufw, swap, log rotation, unattended upgrades.
...
7. **Backups on the new box**: nightly `pg_dump` cron as before, off-box copy to
   Cloudflare R2 (`rclone`, free tier) — or rely on Hetzner's server Backups for the
   first week and add R2 after.
...
- **AI read on profiles**: Vertex worked keylessly because the VM was inside Google.
  On Hetzner the code's fallback is the Anthropic provider (`ANTHROPIC_API_KEY`,
...
- **Region**: us-central1 → Ashburn (US East). Cloudflare fronts both; the origin hop
  changes by ~20 ms either way for US users.
- **IPv6**: Hetzner gives one; Cloudflare can carry an AAAA record too, optional.

## Rollback

Until GCP is deleted, rollback is the DNS edit in reverse plus `docker compose up -d`
on GCE. Orders placed on Hetzner in between would need a reverse `pg_dump`; the shop
